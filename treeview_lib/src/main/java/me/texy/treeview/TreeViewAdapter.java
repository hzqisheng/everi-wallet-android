...
public class TreeViewAdapter extends RecyclerView.Adapter {
    ...
    public int getItemViewType(int position) {
        ...
        int isLeaf;
        if (expandedNodeList.get(position).isLeafNode()) { //是否是叶子节点，1是、0不是
            isLeaf = 1;
        } else {
            isLeaf = 0;
        }
    ...
    @NonNull
    @Override
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int isLeaf) {
        View view = LayoutInflater.from(context).inflate(baseNodeViewFactory
                .getNodeViewBinder(EMPTY_PARAMETER, isLeaf).getLayoutId(), parent, false);

        BaseNodeViewBinder nodeViewBinder = baseNodeViewFactory.getNodeViewBinder(view, isLeaf);
